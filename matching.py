...
def _not_converged(men_engage: Dict[str, Optional[str]],
...
def _propose(man: str,
    ...
    if women_engage[woman] is None:
        logger.info('%s is currently unengaged and accepts %s', woman, man)
    ...
    if women_pref[woman].index(man) < women_pref[woman].index(current_dude):
        ...
        logger.info(
            '%s accepts proposal from %s, leaving %s unengaged',
            woman,
            man,
            current_dude)
    else:
        logger.info(
            '%s declines proposal from %s, staying with %s',
            woman,
            man,
            current_dude)


def _match_validate(men_engage: Dict[str, Optional[str]],
                    women_engage: Dict[str, Optional[str]],
    ...
    logger.info('Checking stability...')
    # Check stability.
...
def compute_smp(men_pref: Dict[str, List[str]],
    ...
    epoch = 1
    while _not_converged(men_engage, women_engage):
        ...
        for index, man in enumerate(men_pref):
            ...
            if men_engage[man] is not None:
                logger.info('%s is already engaged. Skipped.', man)
                continue

            # Get this man's most preferred woman and increment tracker.
            mi_amor = men_pref[man][man_track[index]]
            man_track[index] += 1
            logger.info('%s proposes to %s', man, mi_amor)

            # Propose to her and make relevant state changes.
            _propose(man, mi_amor, women_pref, men_engage, women_engage)

    logger.info('Converged before epoch %d', epoch)
    # We should now have a stable matching.
    _match_validate(men_engage, women_engage, men_pref, women_pref)
